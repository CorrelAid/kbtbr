...
file <- upload("./xls_form_via_post.xlsx")
crul_resp <- crul_client$post(body = list(y=file),encode = "multipart")
crul_resp$raise_for_status()

# status 200 but the new form didn't appear on the kobo website


### 2. deploy an asset
# Build the URL:
asset_depl_url <- crul::url_build(
  url = base_url,
  #path = "api/v2/assets/ajzghKK6NELaixPQqsm49e/deployment",
  path = "assets/ajzghKK6NELaixPQqsm49e/deployment",
  query = list(active=TRUE)
)
...
crul_resp <- crul_client$post()
crul_resp$raise_for_status() #404
