...
### 1. POST request to submit submit XLS form
crul_resp_form <- crul_client$post(
  path = "imports/",
  # path = "api/v2/imports/", # for some reason this doesn't work - 404
  body = list(file=upload("./xls_form_via_post.xlsx"),
              library = 'false')
  )
crul_resp_form$raise_for_status() # status 200 but the new form didn't appear on the kobo website


### 2. POST request to deploy an asset
crul_resp_depl <- crul_client$post(
  path = "api/v2/assets/ajzghKK6NELaixPQqsm49e/deployment/",
  body=list(active=TRUE)
  )
crul_resp_depl$raise_for_status()


### 3. POST request to clone a form
crul_resp_clone <- crul_client$post(
  path = "api/v2/assets/",
  body=list(clone_from="ajzghKK6NELaixPQqsm49e",
            name='My Cloned Form')
  )
crul_resp_clone$raise_for_status() # status 200 but the new form didn't appear on the kobo website




## Try the same with httr
### 1. POST request to submit XLS form
result <- httr::POST(url=paste0(base_url,"imports/"),
                     body=list(file=upload_file("./xls_form_via_post.xlsx"),
                               library = 'false'),
                     add_headers(Authorization = paste0("Token ", token),
                                 Accept = "application/json")
                     ) # again status 200 and nothing happens on the website o_O

### 2. POST request to deploy an asset
result1 <- httr::POST(url=paste0(base_url,"api/v2/assets/ajzghKK6NELaixPQqsm49e/deployment/"),
                      body=list(owner=paste0("kobo.correlaid.org/users/api_user/"),
                                active=TRUE),
                      add_headers(Authorization = paste0("Token ", token),
                                  Accept = "application/json")
                     ) # 404
